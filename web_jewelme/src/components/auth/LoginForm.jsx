    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (data?.captchaRequired) {
          ...
          setCaptchaToken(null); // Reset captcha for retry
          setErrorMsg("Please verify CAPTCHA");
        } else if (err?.response?.status === 403 && data?.code === "EBADCSRFTOKEN") {
      ...
      <div className="w-1/2 flex items-center justify-center p-10">
        <div className="w-full max-w-md space-y-4">
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-red-500 hover:bg-red-600 text-white py-2 rounded disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isLoading}
            >
